...
An intervention consists in replacing one or more of the equations presents in a Structure Equation Model (SEM). It's basically changing the causal relationship between the variables of our problem. A _valid intervention_ is one that does not destroy too much the causal relationship involving $$Y$$, the variable of interest we're trying to predict.
